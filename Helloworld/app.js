document.addEventListener("DOMContentLoaded", () => {
  ...
  const sfx = {
    click(){ beep(520, .05, "square", .05); },
    select(){ beep(640, .06, "triangle", .06); },
    roll(){ beep(220, .08, "sawtooth", .04); setTimeout(()=>beep(260,.08,"sawtooth",.04), 90); },
    win(){ beep(880,.07,"triangle",.07); setTimeout(()=>beep(1040,.08,"triangle",.08), 90); },
    lose(){ beep(180,.10,"sine",.08); setTimeout(()=>beep(140,.12,"sine",.08), 120); },
  };
  ...
  async function roll(){
    ...
    const win = (state.pick === "even" && isEven) || (state.pick === "odd" && !isEven);
  ...
  rollBtn.addEventListener("click", ()=>{ sfx.click(); roll(); });
